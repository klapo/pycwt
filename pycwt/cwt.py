...
class CWT:
    ...
    def signal_norm(self):
        """Return the normalized signal used to perform the cwt."""
        return self._signal_norm

    @property
    def dx(self):
        """Return the coordinate step size."""
        return self._dx
    ...
    @property
    def power(self):
        """Return the cwt power spectrum."""
        return self._power

    @property
    def fft_scales(self):
        """Return the inverse fft frequencies."""
        return self._scales

    @property
    def period(self):
        """Return the inverse cwt frequencies."""
        return self._period

    @property
    def sig_lvl(self):
        """Return the level used for significance testing."""
        return self._sig_lvl

    @property
    def coi(self):
        """Return the cwt's cone of influence."""
        return self._coi

    @property
    def significance(self):
        """Return the significance array (either an array of indices where significant,
        or an array of significance values).
        """
        return self._significance

    @property
    def global_significance(self):
        """Return the global significance array."""
        return self._global_significance

    @property
    def scale_avg(self):
        """Return the scale averaged cwt power."""
        return self._scale_avg

    @property
    def scale_avg_significance(self):
        """Return the scale averaged cwt power significance."""
        return self._scale_avg_significance
    ...
    def fit(
        ...
        if self._significance_test and "sig_lvl" in self._sig_kwargs:
            self._sig_lvl = self._sig_kwargs["sig_lvl"]
        else:
            self._sig_lvl = 0.95
        ...
        self._signal_norm = standardize_data(signal, **self._norm_kwargs)
        ...
        if self._significance_test:
            if "alpha" not in self._sig_kwargs:
                # Lag-1 auto-correlation for red noise-based significance testing
            ...
            signif, _ = pycwt.significance(
                ...
                0,
                alpha,
                significance_level=self._sig_lvl,
        ...
        # Just return nans in the significance variables as they won't break e.g.
        # plotting routines.
        else:
            self._global_significance = np.ones_like(self._global_power) * np.nan
            self._significance = np.ones_like(self._power) * np.nan

        # @ToDo: determine if this step is necessary since we perform rectification above.
        if self._rectify:
        ...
        if self._scales_to_avg is not None:
            ...
            for nint in np.arange(num_intervals):
                # Calculate the scale average between two periods and the significance
                # level.
                sel = find(
                ...
                # In the pycwt example and TC98 eq. 24 the scale-avg power is scale
                # rectified. However, the rectification occurs above in the calculation
                # of power. Do not repeat here.
                if self._rectify:
                ...
                # we return the rectified power here.
                    ...
                    # The example code is just included here as a reference scale_avg =
                    # (scales * np.ones((N, 1))).transpose() scale_avg = power /
                    # scale_avg  # As in Torrence and Compo (1998) equation 24 TC98 eq.
                ...
                if self._significance_test:
                    try:
                        ...
                        # scales. Return no significance level for this averaging
