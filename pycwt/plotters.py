...
def plot_periodogram(
    ...
    # Normalized wavelet power spectrum and significance level contour lines and cone
    # of influence hatched area. Note that period scale is logarithmic.
    ...
    if levels is None and log_power:
        ...
        levels = np.linspace(lq, uq, 10, endpoint=True)
    ...
    if sig95 is not None:
        ax.contour(
            x, np.log2(period), sig95, [-99, 1], colors="k", linewidths=2, extent=extent
        )
    if coi is not None:
        ax.fill(
            np.concatenate([x, x[-1:] + dx, x[-1:] + dx, x[:1] - dx, x[:1] - dx]),
            np.concatenate(
                [
                    np.log2(coi),
                    [1e-9],
...
def plot_wv_power(
    ...
    power = cwt_fit.power
    x = cwt_fit.x
    signal_norm = cwt_fit.signal_norm
    dx = cwt_fit.dx
    ...
    scale_avg_signif = cwt_fit.scale_avg_significance
    ...
    if not coherence:
        ...
        bax.set_title(
            "b) {} Wavelet linear coherence ({})".format(label, mother), loc="left"
        )
    ...
    if scale_avg is not None:
        ...
        for nint in np.arange(num_intervals):
            label = "{bs}{p_units} to {ts}{p_units}".format(
                bs=avg_scales[nint][0], ts=avg_scales[nint][1], p_units=scale_units
            )
            ...
            dax.plot(
                x, scale_avg[:, nint], color=colors[nint], linewidth=1.5, label=label
            )
